...
void handleRoot() {
  ...
  server.sendContent(
    "<html><head><title>CAPTIVE PORTAL - GREEN PLUG</title></head><body>"
    "<h1>PÁGINA DE CONFIGURACAO TOMADA VERDE-IC!</h1>"
  );
...
void handleWifiSave() {
  ...
  server.client().stop(); // Stop is needed because we sent no content length
  saveCredentials();
  has_ssid = strlen(ssid) > 0; // Request WLAN connect with new credentials if there is a SSID
}
